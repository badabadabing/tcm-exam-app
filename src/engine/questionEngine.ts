...
import { resolveDemographics } from './demographics'
import type { Disease, Question, QuestionOption, QuestionType, Syndrome } from '../types'
import { pickManyUnique, pickOne, randomInt, shuffle } from '../utils/random'
...
function getDisease(diseases: Disease[], disease_id: string): Disease | undefined {
...
function buildCasePrefix(disease: Disease, syndrome: Syndrome, symptom_display: string): string {
  const d = resolveDemographics(disease, syndrome)
  const age_str = d.age_unit === '个月' ? `${d.age}个月` : `${d.age}岁`
  const extra = d.extra_context ? `，${d.extra_context}` : ''
  return (
    `${d.title}，${d.gender}，${age_str}${extra}。` +
    `${disease.disease_name}相关症状${d.duration}，${d.season}发病。现症见：${symptom_display}。`
  )
}

function buildExplanation(disease: Disease, syndrome: Syndrome, correct_answer: string) {
...
function generateByType(dataset: QuestionDataset, syndrome: Syndrome, question_type: QuestionType): Question {
  const disease = getDisease(dataset.diseases, syndrome.disease_id)
  if (!disease) {
  ...
  const case_prefix = buildCasePrefix(disease, syndrome, sampled_text)
  ...
  return {
    ...
    explanation: buildExplanation(disease, syndrome, correct_answer),
  }
}
